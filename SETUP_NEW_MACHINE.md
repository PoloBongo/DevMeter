...
the new machine (in an empty folder, or anywhere — no repo clone needed,
the collector installs from npm).
...
3. Run `npm install -g devmeter-cli` to expose the global `devmeter`
   command.
...
5. Set up a shell alias so plain `claude` transparently routes through
   ...
   - **bash/zsh**: append to `~/.bashrc` or `~/.zshrc`:
     ...
     ```
6. Tell me to open a new terminal for the alias to take effect, and that
   after that I can just use `claude` normally — no `devmeter start`
   needed, every session tracks itself.

This is collector-only setup on a machine that just needs to report to the
already-deployed app — it never touches `web/` or the Neon database.
